...
\textbf{Model:} The model classes within SwiftUI are used as a data container that define a structure that data will be written to and fetched from. An example of 
...
skill, an image for the card, and gradientColours with defines the colour scheme for that skill card. The model and it's data are similarly used to display the
statistics for each skill in the Statistics page. 

A model is also created to hold the note entries created by the user via CoreData, the permanent storage 
organising the data in an entity-attribute relationship. Using Core Data involved a steep learning curve during implementation, however, proved to be 
beneficial due to the ability to manipulate the data within the fetch request in a single line of code, making for greater simplicity
