...
SwiftUI presents both very useful features and challenges when creating an application. Instead of the typical Model View Controller design pattern, SwiftUI
...
\textbf{Model:} The model classes within SwiftUI are used as a data container that define a structure that data will be written and fetched from. An example of 
this is the Skill Model. This model defines that each skill displayed on the skill cards has an id, a title, a headline containing the description of the 
skill, an image for the card, and gradientColours with defines the colour scheme for that skill card. The model and it's data are also used to display the
statistics for each skill in the Statistics page. A model is also created to hold the note entries created by the user via CoreData, the permanent storage 
organising the data in an entity-attribute relationship. Using Core Data proved to be a steep learning curve during implementation, however, proved to be 
beneficial due to the ability to manipulate the data within the fetch request which can be done in a single line of code making for greater simplicity
whilst implementing, and it's ability to track changes through the persistant storage. CoreData is also saved using an NSManagedObject which uses an optimised
storage and retrieval methods with key-value coding and observing, this is faster than other means of data storage and retrieval including get/set methods 
...
user interface; text, images, shapes, navigation bars and any element that appears on the screen. The controls allow the user to interact with this interface.
...
and updating the UI state and behaviour accordingly. The ViewModel can take form of the @State variables that provide the binding for the view, updating the 
...
ObservableObject that controls which view is displayed on the UI as the user navigates through the application, as the user clicks on a navigation button the 
...
https://betterprogramming.pub/swiftui-and-coredata-the-mvvm-way-ab9847cbff0f talk about how i could have fetched data differently.
